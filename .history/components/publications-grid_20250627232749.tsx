...
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from "@/components/ui/dialog"
import { ExternalLink, Quote, FileText, Search, Copy, Check, Download, ChevronDown, ChevronUp } from "lucide-react"
import { toast } from "@/hooks/use-toast"
  ...
  const publications = [
    {
      ...
      bibtex: `@article{bhavinlal2025influence,
  title={Influence of Blending Parameters on Temperature Susceptibility of Rice Husk Ash Modified Asphalt},
  author={Bhavinlal, K and Venudharan, V},
  journal={Transportation in Developing Economies},
  year={2025},
  doi={10.1007/s40890-024-0123-4}
}`,
          ...
          filteredPublications.map((publication, index) => (
            ...
            >
              <CardContent className="p-6">
                ...
                <div className="mb-2">
                  <p className="text-gray-700 font-medium">Journal:</p>
                  <p className="text-gray-600">{publication.journal}</p>
                </div>

                <div className="mb-2">
                  <p className="text-gray-700 font-medium">Volume:</p>
                  <p className="text-gray-600">{publication.volume}</p>
                </div>

                <div className="mb-2">
                  <p className="text-gray-700 font-medium">Issue:</p>
                  <p className="text-gray-600">{publication.issue}</p>
                </div>
                ...
                <div className="mb-2">
                  <p className="text-gray-700 font-medium">Publisher:</p>
                  <p className="text-gray-600">{publication.publisher}</p>
                </div>

          {expandedIndex === index ? (
            <>
              <p className="text-gray-600 text-sm leading-relaxed mb-6">{publication.abstract}</p>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="bg-gray-100 p-4 rounded-lg relative">
                          <pre className="text-sm overflow-x-auto">{publication.bibtex}</pre>
                          <Button
                            size="sm"
                            variant="outline"
                            className="absolute top-2 right-2"
                            onClick={() => handleCiteBibtex(publication.bibtex)}
                          >
